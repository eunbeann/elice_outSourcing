/* ========================================================
 * !!! 안내 !!!
 * 해당 파일은 예시 실습 파일입니다.
 * 실습 자료 제작에 대한 자세한 내용은 프로젝트 README.md를 참고해주세요.
 *
 * !!! Notice !!!
 * This is an example exercise file.
 * For more information on creating exercise materials,
 * please refer to the project README.md.
 * ======================================================== */

import { createEliceExtRoutes } from '@elice/extcontent-utils';

//
//
//

export default createEliceExtRoutes([
  {
    path: 'quiz1-1-1',
    component: import('./Six-one/day1/P111'),
  },
  {
    path: 'quiz1-1-2',
    component: import('./Six-one/day1/P112'),
  },
  {
    path: 'quiz1-1-3',
    component: import('./Six-one/day1/P113'),
  },
  {
    path: 'quiz1-2-1',
    component: import('./Six-one/day2/P121'),
  },
  {
    path: 'quiz1-2-2',
    component: import('./Six-one/day2/P122'),
  },
  {
    path: 'quiz1-2-3',
    component: import('./Six-one/day2/P123'),
  },
  {
    path: 'quiz1-3-1',
    component: import('./Six-one/day3/P131'),
  },
  {
    path: 'quiz1-3-2',
    component: import('./Six-one/day3/P132'),
  },
  {
    path: 'quiz1-3-3',
    component: import('./Six-one/day3/P133'),
  },
  {
    path: 'quiz1-4-1',
    component: import('./Six-one/day4/P141'),
  },
  {
    path: 'quiz1-4-2',
    component: import('./Six-one/day4/P142'),
  },
  {
    path: 'quiz1-4-3',
    component: import('./Six-one/day4/P143'),
  },
  {
    path: 'quiz1-5-1',
    component: import('./Six-one/day5/P151'),
  },
  {
    path: 'quiz1-5-2',
    component: import('./Six-one/day5/P152'),
  },
  {
    path: 'quiz1-5-3',
    component: import('./Six-one/day5/P153'),
  },
  {
    path: 'quiz2-1-1',
    component: import('./Six-two/day1/P211'),
  },
  {
    path: 'quiz2-1-2',
    component: import('./Six-two/day1/P212'),
  },
  {
    path: 'quiz2-1-3',
    component: import('./Six-two/day1/P213'),
  },
  {
    path: 'quiz2-2-1',
    component: import('./Six-two/day2/P221'),
  },
  {
    path: 'quiz2-2-2',
    component: import('./Six-two/day2/P222'),
  },
  {
    path: 'quiz2-2-3',
    component: import('./Six-two/day2/P223'),
  },
  {
    path: 'quiz2-3-1',
    component: import('./Six-two/day3/P231'),
  },
  {
    path: 'quiz2-3-2',
    component: import('./Six-two/day3/P232'),
  },
  {
    path: 'quiz2-3-3',
    component: import('./Six-two/day3/P233'),
  },
  {
    path: 'quiz2-4-1',
    component: import('./Six-two/day4/P241'),
  },
  {
    path: 'quiz2-4-2',
    component: import('./Six-two/day4/P242'),
  },
  {
    path: 'quiz2-4-3',
    component: import('./Six-two/day4/P243'),
  },
  {
    path: 'quiz2-5-1',
    component: import('./Six-two/day5/P251'),
  },
  {
    path: 'quiz2-5-2',
    component: import('./Six-two/day5/P252'),
  },
  {
    path: 'quiz2-5-3',
    component: import('./Six-two/day5/P253'),
  },
  {
    path: 'quiz3-1-1',
    component: import('./Six-three/day1/P311'),
  },
  {
    path: 'quiz3-1-2',
    component: import('./Six-three/day1/P312'),
  },
  {
    path: 'quiz3-1-3',
    component: import('./Six-three/day1/P313'),
  },
  {
    path: 'quiz3-2-1',
    component: import('./Six-three/day2/P321'),
  },
  {
    path: 'quiz3-2-2',
    component: import('./Six-three/day2/P322'),
  },
  {
    path: 'quiz3-2-3',
    component: import('./Six-three/day2/P323'),
  },
  {
    path: 'quiz3-3-1',
    component: import('./Six-three/day3/P331'),
  },
  {
    path: 'quiz3-3-2',
    component: import('./Six-three/day3/P332'),
  },
  {
    path: 'quiz3-3-3',
    component: import('./Six-three/day3/P333'),
  },
  {
    path: 'quiz3-4-1',
    component: import('./Six-three/day4/P341'),
  },
  {
    path: 'quiz3-4-2',
    component: import('./Six-three/day4/P342'),
  },
  {
    path: 'quiz3-4-3',
    component: import('./Six-three/day4/P343'),
  },
  {
    path: 'quiz3-5-1',
    component: import('./Six-three/day5/P351'),
  },
  {
    path: 'quiz3-5-2',
    component: import('./Six-three/day5/P352'),
  },
  {
    path: 'quiz3-5-3',
    component: import('./Six-three/day5/P353'),
  },
  {
    path: 'quiz4-1-1',
    component: import('./Six-four/day1/P411'),
  },
  {
    path: 'quiz4-1-2',
    component: import('./Six-four/day1/P412'),
  },
  {
    path: 'quiz4-1-3',
    component: import('./Six-four/day1/P413'),
  },
  {
    path: 'quiz4-2-1',
    component: import('./Six-four/day2/P421'),
  },
  {
    path: 'quiz4-2-2',
    component: import('./Six-four/day2/P422'),
  },
  {
    path: 'quiz4-2-3',
    component: import('./Six-four/day2/P423'),
  },
  {
    path: 'quiz4-3-1',
    component: import('./Six-four/day3/P431'),
  },
  {
    path: 'quiz4-3-2',
    component: import('./Six-four/day3/P432'),
  },
  {
    path: 'quiz4-3-3',
    component: import('./Six-four/day3/P433'),
  },
  {
    path: 'quiz4-4-1',
    component: import('./Six-four/day4/P441'),
  },
  {
    path: 'quiz4-4-2',
    component: import('./Six-four/day4/P442'),
  },
  {
    path: 'quiz4-4-3',
    component: import('./Six-four/day4/P443'),
  },
  {
    path: 'quiz4-5-1',
    component: import('./Six-four/day5/P451'),
  },
  {
    path: 'quiz4-5-2',
    component: import('./Six-four/day5/P452'),
  },
  {
    path: 'quiz4-5-3',
    component: import('./Six-four/day5/P453'),
  },
]);
